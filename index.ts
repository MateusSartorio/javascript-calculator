...
let text: string = "0";
let result: string = "0";
let justCalculated: boolean = true;
...
const computeResult = (txt: string): string => {

    const numberRegex = /[0-9]*\.{0,1}[0-9]+|[-+\\*/]/g;

    let arr: string[] | null = txt.match(numberRegex);
...
const removeExcessiveOperations = (array: string[]): string[] => {
    let newArray = [];
...
const isOperation = (str: string): boolean => {
    if(str === "+" || str === "-" || str === "*" || str === "/")
        return true;
